    ...
    new GoogleStrategy({
        ...
        try {
            ...
            if (!user) {
                ...
                if (checkUser) {
                    if ((!checkUser.image || checkUser.image.split('/').pop() === 'user-avatar.jpg') && profile._json.picture) checkUser.image = profile._json.picture
            ...
            const Token = token.accessToken(user?._id, user?.role!)
            done(null, {Token});
        } catch (err) {
            done(err, false)
        }
